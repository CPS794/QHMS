...
$(document).ready(function() {
    ...
    getUserList(Enum_UserType.admin);
});
...
function datatableModifySubmit(modifyResult) {
    ...
    var dataString = "userId=" + modifyResult.id + "&userName=" + modifyResult.name + "&userType=" + Enum_UserType.admin;
    $.ajax({
        type: "POST",
            ...
            success: {
                ...
                callback: function() {
                    if ($("#newPassword").val() == $("#confirmPassword").val()) {
                        modifyAdminPassword(data[0].id, data[0].type, $("#newPassword").val(), id);
                    } else {
...
function modifyAdminPassword(userId, userType, newPwd, id) {
    ...
    bootbox.dialog({
        ...
        buttons: {
            success: {
                ...
                callback: function() {
                    if ($("#newPassword").val() == $("#confirmPassword").val()) {
                        addUser($("#DataTableAddUser_id").val(), $("#DataTableAddUser_name").val(), Enum_UserType.admin, $("#newPassword").val());
                    } else {
...
function addUser(userId, userName, userType, password) {
    passwordTip = "";
    password = cal_md5(password);
    var dataString = "userId=" + userId + "&userName=" + userName + "&userType=" + userType + "&password=" + password;
    $.ajax({
        type: "POST",
        url: "addUser.action",
        data: dataString,
...
function deleteUsers(id) {
    ...
    if (rows.length == 0) {
        ...
        if (r == true) {
            var dataString = "userType=" + Enum_UserType.admin + "&deleteIds=" + deleteIds;
            $.ajax({
                type: "POST",
...
function importAdmin(columns) {
    $.ajax({
        type: "POST",
        url: "importAdmin.action",
