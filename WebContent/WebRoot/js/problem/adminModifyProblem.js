...
function initData() {
    $.ajax({
        type: "POST",
        url: "getProblem.action",
        data: "problemId=" + $('#hiddenProblemId').val(),
        dataType: "json",
        success: function(data, textStatus, jqXHR) {
            if (data.success) {
                problem = data.problem;
                $('#AddProblemId').val(problem.id);
                $('#AddProblemChapter').val(problem.chapterId);
                $('#AddProblemType').val(problem.typeId);
                setDisplayArea(anti_types[$('#AddProblemType').val()]);
                $('#AddProblemPoint').val(problem.point);
                $('#AddProblemDescription').summernote('code', problem.description);
                var targets = ["target_" + problem.typeId, eval(problem.option)];
                doFunction(eval("setOption" + problem.typeId), targets);
                targets = ["target_" + problem.typeId, eval(problem.answer)];
                doFunction(eval("setAnswer" + problem.typeId), targets);
            } else {
                alert(data.message);
            }
        },
